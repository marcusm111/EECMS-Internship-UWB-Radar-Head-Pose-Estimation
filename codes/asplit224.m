function asplit224(sensor_id, data_files, K, ovlap, len, offset, fps, mindb, maxdb, run_id,cam_fps,recording_duration)
        ...
        %}
            ...
            time_doppler_map_db_resize = imresize(time_doppler_map_db, [size(time_doppler_map_db,1), T], 'bilinear');
            % Round to 2 decimal places for ML efficiency
            time_doppler_map_db_resize = round(time_doppler_map_db_resize * 100) / 100;
    
            %% Split into T segments
            %{
            for t = 1:T
                spectrogram_segment = time_doppler_map_db_resize(:, t);
                % Save each segment
                ...
                %{
                figure();
                plot(spectrogram_segment);
                ylim([-35 -5])
                %}
            end
            %}
            output_filename = sprintf('%s%sr%d.csv', sensor_id, labels{pos_idx}, run_id);
            writematrix(time_doppler_map_db_resize, output_filename);
            disp(['Saved processed data to ', output_filename]);

            %% Plot with improved title (MODIFIED)
            time_axis = linspace(0, T, T);
            doppler_axis = linspace(-K/2,K/2,K);
            subplot(2, 4, pos_idx);
            imagesc(time_axis, doppler_axis, time_doppler_map_db_resize);
            axis xy;
            colormap jet;
            colorbar;
            title(sprintf('md(%s%sr%d)', sensor_id, labels{pos_idx}, run_id)); % Modified title
            xlabel('Frame');
            ylabel('Doppler Bin');
            clim([mindb, maxdb]);
